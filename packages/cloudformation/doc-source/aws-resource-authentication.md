...
- Unlike most CloudFormation resources, the `AWS::CloudFormation::Authentication` type doesn't contain a block called _Properties_, but instead contains a list of user\-named blocks, each containing its own authentication properties\.
...
_Required_: Conditional\. Can be specified only if the `type` property is set to `"S3"`\.  
_Type_: String
...
_Required_: Conditional\. Can be specified only if the type property is set to `"basic"`\.  
_Type_: String
...
This template snippet shows how to get a file from a private S3 bucket within an EC2 instance\. The credentials used for authentication are defined in the `AWS::CloudFormation::Authentication` resource, and referenced by the `AWS::CloudFormation::Init` resource in the _files_ section\.
...
The following example template snippet includes both _basic_ and _S3_ authentication types\.
