...
Also, before you begin, make sure you have an Amazon EC2 key pair in the region in which you're creating your stack\. For more information, see [Amazon EC2 key pairs](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-key-pairs.html) in the _Amazon EC2 User Guide for Linux Instances_\.
...
1. Add the web server configuration metadata to the `WebServerInstance` resource\.
   ...
   1. _If you are authoring your template in JSON_: Within the `Metadata` braces \(`{}`\) and after the `AWS::CloudFormation::Designer` closing brace, add a comma \(`,`\)\.
