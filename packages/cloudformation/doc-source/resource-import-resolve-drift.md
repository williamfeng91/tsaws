...
In this example, let's assume a user changed a resource _outside_ of CloudFormation\. After running drift detect, we discovered that `GamesTable` has been modified `BillingMode` to `PAY_PER_REQUEST`\. For more information about drift detect, see [Detecting unmanaged configuration changes to stacks and resources](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/using-cfn-stack-drift.html)\.
...
1. On the **Review _stack\-name_** page, choose **Update stack**\.

_Results_: On the **Events** page of your stack, the status is `UPDATE_COMPLETE`\.
...
_Results_: The **Logical ID** `GamesTable` has a status of `DELETE_SKIPPED` on the **Events** page of your stack\.
